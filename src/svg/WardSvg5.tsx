...
interface ColorableSvgProps extends SvgProps {
...
const SvgWardSvg5: React.FC<ColorableSvgProps> = ({
  colorMap = {},
  currentColor = '#000',
  onElementPress,
  onColorChange,
  ...
  const enhanceElement = (element: React.ReactElement<any>, id: string) => {
    ...
    const fillColor = colorMap[id] || element.props.fill || 'none';
    return React.cloneElement(element, {
      fill: fillColor,
      stroke: strokeColor,
      strokeWidth,
      onPress: () => {
        if (selectionMode === 'color' && onColorChange) {
          onColorChange(id, currentColor || fillColor);
        }
  ...
  const processChildren = (
    ...
    return React.Children.map(children, child => {
      ...
      const childId = childProps.id || parentId;
      const shouldEnhance =
        childId &&
      ...
      if (
        ...
        const updatedChildren = processChildren(
          (child.props as { children?: React.ReactNode }).children,
          childId
        );
        processed = React.cloneElement(child, {}, updatedChildren);
      }
      if (childId && shouldEnhance) {
        return enhanceElement(processed, childId);
      }
      return processed;
    });
  };
