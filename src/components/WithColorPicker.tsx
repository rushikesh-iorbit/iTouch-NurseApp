// src/components/WithColorPicker.tsx
import React, { useState } from 'react';
import { View, Text, useWindowDimensions, StyleSheet } from 'react-native';
import { ColorPicker } from './ColorPicker';
import { ColorableSvgProps } from '../types/svg';

interface WithColorPickerProps {
  children: React.ReactElement<ColorableSvgProps>;
  initialColor?: string;
}
...
export const WithColorPicker: React.FC<WithColorPickerProps> = ({ 
  ...
  const handleElementPress = (id: string) => {
    if (selectionMode === 'element') {
      setColorMap(prev => ({ ...prev, [id]: currentColor }));
