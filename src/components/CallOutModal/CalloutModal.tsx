...
import { Modal, View, StyleSheet, Text, Pressable, Image, TouchableOpacity , Dimensions} from 'react-native';
...
const CalloutModal: React.FC<CalloutModalProps> = ({ visible, onClose, bedPatientInfo }) => {
  ...
  const modalWidth = width * 0.8;  // leaves 10% on each side
  const modalHeight = height * 0.86; 
      ...
      <View style={styles.overlay}>
        <Pressable style={styles.backdrop} onPress={onClose} />
         <View style={[styles.card, { width: modalWidth, height: modalHeight }]}>
          ...
          <View style={{ flexDirection: 'row', marginTop: 20, gap: 12, alignItems: 'flex-start' }}>
            ...
            <View style={{ width: '48%', justifyContent: 'flex-start', alignSelf: 'flex-start' }}>
              {selectedTab === 'instructions' && (
                <View style={[styles.instructionSection, { justifyContent: 'flex-start', alignItems: 'flex-start' }]}>
              ...
              {selectedTab === 'moveout' && (
                <View style={styles.instructionSection}>
                  <View style={styles.instructionHeaderRow}>
                    ...
                    <Text style={[styles.instructionsHeader, { color: '#4CAE51' }]}>Move Out</Text>
                  </View>                  
                  ...
                  <TouchableOpacity style={styles.confirmButton}>
                    <Text style={{ color: '#000', fontWeight: '600' }}>Yes</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
...
const ActionButton = ({ label, icon, onPress, isActive }: { label: string; icon: any; onPress: () => void; isActive: boolean }) => (
  ...
  >
    ...
    <Text style={[styles.actionButtonText, { color: isActive ? '#fff' : '#1E4721' }]}>{label}</Text>
  </TouchableOpacity>
);
...
closeText: {
  ...
  borderColor: '#ccc',
  gap: 8,
  alignItems: 'flex-start', // aligns text left
